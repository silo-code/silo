import type { GitRepoStore } from "./git-repo-store";

// The `silo.git` provider's public contract. Consumers (the git-explorer view,
// the diff editor, first-party `core.*` extensions, and third-party
// extensions alike) import these types at build time and retrieve the live
// implementation at runtime via `ctx.getExtension<GitAPI>("silo.git").api`.
// Ships from its own package (`@silo-code/git-api`, not the core SDK barrel)
// per ADR 0009: git is an extension-owned feature, not a core primitive, so
// its API ships alongside the extension rather than through `@silo-code/sdk`.
// See ADR 0037 for `watchRepo` and the live `GitRepoStore` session this
// package also publishes.
 ...
 * off the UI thread via `ctx.process.exec`. Mutating calls resolve to `void`
 * (output is not surfaced); read calls resolve to parsed data. See
 * {@link GitAPI.watchRepo} for the live, subscribable counterpart to these
 * one-shot reads.
 */
export interface GitAPI {
  ...
  /**
   * A live, deduplicated {@link GitRepoStore} for `cwd`. Idempotent by
   * normalized path — repeated calls with the same `cwd` return the identical
   * object, so callers may call this directly in render without `useMemo`.
   * See ADR 0037.
   */
