...
export function renderTrackSessionScript(
  ...
  return [
    ...
    `# Marker: ${marker}`,
    "",
    "# Hook Guard (RFC 0028): inert outside a Silo terminal. $SILO is stamped",
    "# once at session creation and cannot be set by a caller (reserved on both",
    "# spawn and exec), so this can't be spoofed from inside the shell. A tab",
    "# left open across an in-place upgrade has no $SILO yet and goes quiet",
    "# until it's closed and reopened — accepted rather than falling back to",
    "# the old unscoped walk, which would defeat the guard for every terminal.",
    '[ -n "$SILO" ] || exit 0',
    "",
    'agent="$1"',
    "payload=$(cat | tr '\\n' ' ')",
