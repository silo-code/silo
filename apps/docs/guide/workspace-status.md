...
| ------------ | -------------------------- | --------------------------------------------------------- |
| **Status**   | Below the path line        | Status rows with a dot, label, and optional elapsed time  |
| **Sections** | Below status rows          | Arbitrary React component, mounted once per workspace row |
...
```ts
import type { WorkspaceStatusProvider } from "@silo-code/sdk";

function activate(ctx) {
  const provider: WorkspaceStatusProvider = {
    id: "my-ext.status",
    provide(workspaceId) {
  ...
  ctx.subscriptions.push(
    ctx.workspaces.registerStatus(provider),
    // re-render whenever your underlying data changes
    myTaskStore.onChange(() => ctx.workspaces.invalidateStatus()),
  );
}
```

**How invalidation works.** The panel doesn't poll — it waits for
`invalidateStatus()`. Call it after any mutation to the state your
`provide` function reads. `subscribeStatus` lets you observe invalidations
...
and any status rows. The component receives the workspace id and can render
...
| Scenario                                                   | Use                               |
| ---------------------------------------------------------- | --------------------------------- |
| "Is a task running? What is it called?"                    | Status row — semantic dot + label |
| "How long has this agent been running?"                    | Status row with `startedAt`       |
...
Status rows and badges are read-only — they show state, they don't take input.
Sections can be fully interactive.

## See also

- [`ctx.workspaces`](/api/state/workspaces) — full method reference
- [`WorkspaceStatusProvider`](/api/types/interfaces/WorkspaceStatusProvider) · [`WorkspaceStatusRow`](/api/types/interfaces/WorkspaceStatusRow)
