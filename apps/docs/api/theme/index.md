...
everything registered via
[`ctx.registerThemePreset`](/api/registration/register-theme-preset)), switch the
active theme, and manage the user's custom themes. The built-in theme picker and
theme editor consume the domain through here rather than reaching into app state.
...
because extensions register and unregister them at runtime. Contributing a preset
is a separate, registration-time concern:
[`ctx.registerThemePreset`](/api/registration/register-theme-preset). Core ships
only Dark and Light as the always-present fallback; the bundled presets (Tokyo
Night, Solarized Light, Gruvbox Dark) live in the `theme-presets` built-in
extension, so the picker's bundled set is itself just contributions.
