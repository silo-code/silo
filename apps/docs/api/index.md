...
| [`ctx.agents.profiles`](/api/agents/profiles) | the user's Agent Profiles — list them, launch one, optionally with an opening prompt ([`AgentProfilesService`](/api/types/interfaces/AgentProfilesService)) <Badge type="warning" text="beta" />                    |
| [`ctx.agents.sessions`](/api/agents/sessions) | connect to a user-authored Chat profile (an Agent Client Protocol child) and prompt / cancel / watch it ([`AgentSessionsService`](/api/types/interfaces/AgentSessionsService)) <Badge type="warning" text="beta" /> |
...
| [`usePanelEntryFocus`](/api/other/use-panel-entry-focus)   | React hook that lands the caret in a dock panel's content on entry — both entry signals, the guard, and the retry            |
