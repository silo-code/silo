...
| [`ctx.workspaces`](/api/state/workspaces) | workspaces &amp; editor tabs; `registerDecoration` to add status rows and `registerSection` to mount React components in workspace rows ([`WorkspaceService`](/api/types/interfaces/WorkspaceService)) |
