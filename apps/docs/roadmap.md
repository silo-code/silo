...
| `ctx.agents` (coding-agent activity + resume hints)                               | <Badge type="warning" text="beta" /> | [How it works](/roadmap/agent-system) — keyed on the **Agent Session** (Terminal or Chat), one `AgentInfo` shape for both; `reveal(id)` / `resume(id)` verbs ([RFC 0038](https://github.com/silo-code/silo/blob/main/docs/proposals/0038-acp-agent-sessions.md))                   |
...
| Agent Profiles (`ctx.agents.profiles`)                                            | <Badge type="warning" text="beta" /> | [docs](/api/agents/profiles) · [RFC 0033](https://github.com/silo-code/silo/blob/main/docs/proposals/0033-agent-profiles.md) — `list()` + `launch({ prompt })`; an opening prompt is delivered as a literal or refused, never mangled                                              |
| Chat sessions (`ctx.agents.sessions`)                                             | <Badge type="warning" text="beta" /> | [docs](/api/agents/sessions) · [RFC 0038](https://github.com/silo-code/silo/blob/main/docs/proposals/0038-acp-agent-sessions.md) — `connect()` a user-authored Chat profile (an Agent Client Protocol child) and `prompt` / `cancel` / watch it; gated on the `chatAgents` setting |
...
| `includeReplay` on `subscribeOutput` / `subscribeOsc` / `onData`                  | <Badge type="tip" text="stable" />   | [docs](/api/state/terminals#replayed-scrollback) — opt in to the scrollback replayed on re-attach, tagged so it isn't read as live activity                                                                                                                                        |
...
| `ctx.storage` directories (`globalDir` / `workspaceDir` / `workspaceDirs`)        | <Badge type="tip" text="stable" />   | [docs](/api/storage/#storage-directories)                                                                                                                                                                                                                                          |
...
| Agent Chat panel       | <Badge type="info" text="planned" /> | `agents.sessions` + dock panel          | — · behind the `chatAgents` setting; built on `ctx.agents.sessions` alone, so disabling it and installing a third-party Chat UI works like any other extension ([RFC 0038](https://github.com/silo-code/silo/blob/main/docs/proposals/0038-acp-agent-sessions.md)) |
...
| CLI Control API         | <Badge type="info" text="planned" />         | [RFC 0034](https://github.com/silo-code/silo/blob/main/docs/proposals/0034-control-api/proposal.md) |
