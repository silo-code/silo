...
| Agent Chat panel       | <Badge type="info" text="planned" /> | `agents.sessions` + dock panel          | — · behind the `bundledChatPanel` setting; built on `ctx.agents.sessions` alone, so a third-party Chat UI can replace it ([RFC 0038](https://github.com/silo-code/silo/blob/main/docs/proposals/0038-acp-agent-sessions.md)) |
