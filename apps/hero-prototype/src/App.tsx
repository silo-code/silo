...
const TERMINAL_DEFAULT_DELAY_MS = 250;
...
// The full catalog (baseWorkspaces) never changes — only which of its
// workspaces are currently "open" (visible in the Navigator, switchable)
// does. build-server and mobile start closed and can be opened — and
// anything open can be closed again — through the Navigator's + menu,
// repeatably, by a real visitor, not just once by the script.
const INITIAL_OPEN_IDS = ["website", "docs", "api"];

/** A small, stable (non-random-per-render) hash — turns an id into a 0–1.39s offset so a set of same-cycle-length pulse animations seeded from different ids don't all pulse in lockstep. */
...
/**
 * Runs an independent, continuously-looping reveal simulation for every
 * terminal-dock tab in every *open* workspace — not just the active one — so
 * agent activity is genuinely live everywhere at once, matching how the real
 * agent-monitor extension watches every workspace regardless of which one is
 * in view. Keys are composite (`${workspaceId}:${tabId}`) since a tab id like
 * "claude" repeats across workspaces.
 *
 * The effect keys off `openWorkspaceIds.join(",")` rather than the workspace
 * objects themselves: `workspaces` state gets a new array reference on almost
 * every interaction (any `updateActive` call), but a terminal's own log
 * content never changes at runtime, so re-deriving tabs from the static
 * `baseWorkspaces` catalog on that stable string keeps every simulation
 * running continuously instead of restarting on unrelated UI clicks.
 */
function useLiveTerminalStates(
  ...
  useEffect(() => {
    ...
    for (const workspace of baseWorkspaces) {
      ...
      const dock = workspace.docks.find((d) => d.kind === "terminal");
      for (const tab of dock?.tabs ?? [])
        liveTabs.push({ key: `${workspace.id}:${tab.id}`, tab });
    }
    ...
    for (const { key, tab } of liveTabs) {
      ...
      if (total === 0) continue;
      // Per-terminal, JSON-authored: how long a finished session sits idle
      // before it starts its next round. A short pause reads as a quick loop; a
      // long one reads as the agent finishing and going idle for a while.
      const loopPause = tab.log.loopPauseMs ?? TERMINAL_LOOP_PAUSE_MS;
      // `before` accumulates prior entries within the current round — a
      // restart doesn't blank the pane, it appends a fresh round after the
      // last one like a real terminal scrolling old output up and out of
      // view. Capped at `TERMINAL_MAX_RETAINED_ENTRIES` at each loop
      // boundary (see `revealFrom(0, ...)` below) so that scrollback effect
      // doesn't grow without bound the longer a tab is left open.
      const revealFrom = (index: number, before: TerminalEntry[]) => {
        // Entry 0's own delayMs is honored too (not forced to 0), so a
        // terminal can linger on its banner before "typing" its first
        // command just by giving that entry a large delayMs.
        const delay =
        ...
        timers.push(
          window.setTimeout(() => {
            ...
            setStates((current) => {
              // Only bump `since` on a genuine status transition — not on every
              // entry reveal while still "working" — so elapsed time reflects
              // how long the current status has actually been true.
 ...
 * its own, driving itself off `script`. Reused for both the full hero (the
 * default `demoScript`) and smaller cropped feature vignettes elsewhere on
 * the page (their own shorter script); each mounted instance gets its own
 * independent state, timers, and terminal simulation, so multiple copies can
 * run on the same page without interfering with one another.
 */
function DemoWorkspace({
  ...
  const [openWorkspaceIds, setOpenWorkspaceIds] =
    useState<string[]>(INITIAL_OPEN_IDS);
  const [activeId, setActiveIdState] = useState(INITIAL_OPEN_IDS[0]);
  const [showWorkspaceMenu, setShowWorkspaceMenu] = useState(false);
  const [navigatorView, setNavigatorView] =
    useState<NavigatorView>("workspaces");
  const [showViewMenu, setShowViewMenu] = useState(false);
  const [playback, setPlayback] = useState({ playing: true, index: 0 });
  ...
  useEffect(() => {
    ...
    if (playback.index >= script.length) {
      const loopTimer = window.setTimeout(() => {
        ...
        setOpenWorkspaceIds(INITIAL_OPEN_IDS);
        setActiveIdState(INITIAL_OPEN_IDS[0]);
  ...
  }, [playback.playing, playback.index, script]);

  const terminalDock = active.docks.find((dock) => dock.kind === "terminal");
  const activeTerminalTab = terminalDock?.tabs?.find(
    (tab) => tab.id === terminalDock.activeTab,
  );
  const liveStates = useLiveTerminalStates(openWorkspaceIds);
  ...
  return (
    ...
    >
      ...
      <div className="demo-body">
        ...
        >
          {!active.collapsed.right && (
            <div className="demo-side-split">
              <DemoSidePane
                panels={active.panels.right}
                activeId={active.activePanels.right}
      ...
      {scriptCursor.visible && (
        <div
          className={`demo-script-cursor ${scriptCursor.clicking ? "is-clicking" : ""}`}
    ...
    <main className="page-shell">
      <header className="site-header">
        ...
        <div className="prototype-label">
          <span className="live-pulse" /> Live workspace prototype
        </div>
        <button className="header-cta" type="button">
          Download Silo <span>↗</span>
        </button>
      </header>

      <section className="intro">
        <p className="eyebrow">THE MULTI-AGENT WORKSPACE</p>
        <h1>
          Every project.
          <br />
          <em>In motion.</em>
        ...
        <p className="intro-copy">
          Switch between complete workspaces while your agents keep moving.
          Click around the live surface below.
        </p>
      </section>

      <DemoWorkspace script={demoScript} focusable />
    </main>
  );
}
  ...
  if (selected.kind === "files")
    return (
      <div className="file-tree">
        ...
        {[".git", "src", "tests", "package.json", "README.md"].map(
          (name, index) => (
            <div
              className={`tree-row ${index === 1 ? "tree-folder" : ""}`}
              key={name}
            >
              <span>{index === 1 ? "⌄" : "›"}</span>
              <span>{name}</span>
        ...
        <div className="tree-row tree-child">◇ {workspace.activeFile}</div>
      </div>
    );
