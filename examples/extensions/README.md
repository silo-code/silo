...
| **[acp-chat](./acp-chat)**                           | A full **dock panel** driving `ctx.agents.sessions` — host-drawn chrome (`toolbar` / `setBreadcrumb`), the `"panel"` toolbar surface, `chatProfileHost`. The Chat panel, shipped as an example (RFC 0038 / 0039). | dock **+** menu → New Agent Chat | `agents`  |
