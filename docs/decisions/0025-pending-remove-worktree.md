...
4. Push a single aggregated, **informational** busy-status entry (RFC 0026
   host StatusBar slot) until the set is empty (not cancelable).
...
- Does **not** ship `ctx.ui.progress`. Pending-remove uses the host **busy
  status** aggregate (`ctx.ui.busyStatus`, @internal) shared with terminal
  restore ([RFC 0026](../proposals/0026-terminal-session-host-backpressure.md))
  — not a private `StatusItem`.
...
- **Private StatusItem for pending-remove** — superseded: second consumer
  (terminal restore) motivated the shared busy-status slot (RFC 0026).

## References

- Glossary: `docs/domain-language.md` (Remove worktree, Pending remove, Busy
  status, Close worktree view).
- Related: [0018](./0018-host-owned-chrome.md) (host chrome; progress still planned).
- Related: [RFC 0026](../proposals/0026-terminal-session-host-backpressure.md)
  (busy status + terminal backpressure).
