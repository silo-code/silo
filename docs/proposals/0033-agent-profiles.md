...
| **3 — Prompt delivery, and `ctx.agents.profiles`** | `promptDelivery` on `AgentDefinition` (all seven agents reconned by running them), the quoted-heredoc / fish-literal transport with line-editor sanitization and a 2 KiB measured limit, the four typed refusals — published through `ctx.agents.profiles` `list()` / `launch({ prompt })`, the phase's real consumer, plus the Profiles-tab affordance. Absorbed what was phase 5. `--prompt` specified, not built (ships with RFC 0034); no CLI code. | **shipped** (2026-09-03) |
