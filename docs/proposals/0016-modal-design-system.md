...
| `Badge`                    | `tone?: "neutral"\|"accent"\|"ok"\|"warn"\|"err"\|"outline"; color?: string`                       | Tones derive from generics (text = tone color, background = 18–20% `color-mix` tint); `accent` uses `--silo-color-accent` (not accent-2); `color` = arbitrary caller color via one custom property (`--badge-color`), same tint derivation — for identity colors (e.g. workspace-group swatches) |
