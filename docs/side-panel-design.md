...
e.currentTarget })`.
  - **Labelled by its current value** — the active filter / grouping itself
    ...
    two prefixed labels eat a narrow toolbar. **Exception — a wide dock
    sheet** (not the panel): when several dropdowns sit clustered in a
    full-width toolbar the bare values stop reading as separate pickers, so
    there each label takes a prefix (`"List: All"`, `"Sort: Title"`,
    `"Status: Open"`, `"Labels: None"`). `silo.tasks`' sheet does this; its
    side panel does not.
...
| 2026-09-01 | **One panel body, three surfaces** — `silo.tasks` renders the same component in the side panel, a Navigator view, and a dock sheet, differing only in _which sources_ it is handed and which chrome it owns (`controls`, `quickAdd` flags). The list styling stays scoped to the one `.tasks-panel` root class; each surface adds a modifier (`.tasks-cross`, `.tasks-sheet`) carrying only its differences.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Two aggregating surfaces that re-implement a panel's list drift within a release. A shared body plus two small modifiers is the cheapest way to keep row vocabulary, grouping, and drill-in identical everywhere.                                                                                                                                                          |
| 2026-09-01 | **A Navigator view's arrange/filter controls are toolbar items, not body chrome** — the body renders only the inline `SearchInput`; `Group by` / `Sort` / lane / label become `"navigator"`-surface `registerToolbarItem`s scoped with `when: (_, t) => t.viewId === <id>` (ADR 0038). The same _dropdown builders_ back both, extracted to a pure module, so the host toolbar and a panel's `MenuButton`s can't drift. Note the asymmetry: a toolbar item's `title` is a static string, so it shows the control's _name_ ("Arrange"), where a panel `MenuButton` shows its current _value_ ("Source").                                                                                                                                                                                                                                                                                                      | The Navigator view body has no header of its own — putting controls in it would stack a second bar under the host's. Sharing menu builders rather than menu chrome is what keeps them one control at two sizes.                                                                                                                                                            |
| 2026-09-01 | **Drop the toolbar's bottom rule when the toolbar is just a search box** (`.tasks-cross .tasks-toolbar`) — with the controls lifted into the host header, the remaining rule sat directly under the host's own and read as a double line.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | A section divider is only a divider when it separates two things; under another rule it is noise.                                                                                                                                                                                                                                                                          |
| 2026-09-01 | **A dock sheet showing a panel's list widens the chrome inset, not the row grammar** — `.tasks-sheet` bumps `--tasks-pad-x` to `12px` and the list container to `8px`, and changes nothing else. Rows, glyphs, and group headers stay byte-identical to the panel's.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | At 720px the panel's 10px/4px staircase leaves rows hugging the edge, but re-tuning row density per surface is how two surfaces stop looking like one feature.                                                                                                                                                                                                             |
| 2026-09-04 | **A Navigator view is a list, not a pager — it hands off to a sheet.** `silo.tasks`' cross-workspace view first drilled into `TaskDetail` in place, matching the side panel; corrected so a row opens the task in the dock sheet instead (`TasksBody`'s `onOpenTask` hands off rather than paging; `initialTask` lands the sheet on that task; a second row re-points the open sheet through its own `PanelBridge`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | The Navigator is the thing you navigate _from_: replacing it with one record's detail removes the list you were working through, and does it in the app's narrowest column. A side panel can page in place because it is already a single-subject surface; a Navigator view is not. Corollary: a view that hands off consumes no `Escape`, so the Navigator keeps its own. |
| 2026-09-04 | **A sheet opened from a Navigator view anchors to the Navigator's panel, not to the extension's own.** `ctx.layout.openPanelSheet` resolves which dock column to grow out of purely from the panel id it is handed (`resolveSidePanelLocation`), so `silo.tasks` passing its own right-side panel opened the sheet on the right while the view sat on the left. Use `NavigatorViewProps.panelId` (SDK 0.46.0+) — never a hard-coded `"navigator"`.                                                                                                                                                                                                                                                                                                                                                                                                                                                           | The anchor has to follow the _click's origin_, and a view is a guest inside someone else's panel. `panelId` is also drag-safe: the host resolves the column the panel is actually in, so moving the Navigator to the right column moves the sheet with it.                                                                                                                 |
| 2026-09-04 | **"Sheets lose inheritance" (above) is easy to half-fix and not notice** — `silo.tasks`' sheet re-applied `base+2px` on its own content root but never passed `className` to `openPanelSheet`, so every `1em` inside the body silently fell back to app base (13px) — a table made it obvious; `TaskDetail` had been rendering a step small the whole time. Fixed: pass `className: "tasks-sheet"` in the `openPanelSheet` opts (`SheetOptions.className` wraps the _whole_ sheet surface, title included, per its own doc comment — confirmed against skills-manager's `openPanelSheet` call), re-apply `calc(var(--silo-font-size-base) + 2px)` on the content root **and** on `.tasks-sheet .silo-sheet-title` (which pins its own absolute `--silo-font-size-base` regardless of the body). Once the ambient is actually correct, no per-component `1em` override anywhere in the sheet needs to change. | Checking a sheet's font scale by eye against a wireframe/mock catches nothing — the pane and the sheet look identical relative to _themselves_, only wrong relative to the rest of the app. Diagnose by tracing whether `className` reached `openPanelSheet` at all, not by eyeballing individual rules.                                                                   |
| 2026-09-07 | **Wide-sheet toolbar dropdowns take a prefix; the side panel's don't** — `silo.tasks`' sheet labels its four dropdowns `"List: All"` / `"Sort: Title"` / `"Status: Open"` / `"Labels: None"`; the same `TasksToolbar` in the side panel keeps bare values. The label mirrors the open dropdown's option: `List: All` ↔ the "All" option, `List: None` ↔ the "No list" option (the unnamed global list). Also: the sheet and the Navigator view now persist their arrange/filter/sort on **separate** keys (`view:sheet` vs `view:cross`), and the sheet's List filter is single-select and not checkable (All / one list / No list).                                                                                                                                                                                                                                                                         | Four dropdowns clustered in a 720px toolbar stop reading as separate pickers without a prefix — the "no prefix" rule was about a _narrow_ toolbar. Sharing one prefs key between a narrow rail and a wide sheet meant a sort chosen in one silently rearranged the other.                                                                                                  |
